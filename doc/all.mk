...
#
WITH_DOC := $(strip $(foreach x,doc.all doc html pdf doxygen install.doc clean test.doc,$(findstring $(x),$(MAKECMDGOALS))))
ifneq "$(WITH_DOC)" ""
...
#
#  We're building a documentation target, but there's no "doxygen".
#
ifeq "$(DOXYGEN)" ""
	$(error doxygen is required to build the source documentation)
endif

#
#  We're building a documentation target, but there's no "dot".
#
ifeq "$(GRAPHVIZ_DOT)" ""
	$(error doxygen needs the command 'dot' provide by graphviz)
endif
...
test.doc:
	${Q}echo TEST-DOC ALL
	${Q}${MAKE} doc.all 3>&1 2>&1 > ${BUILD_DIR}/doc_stderr.log
