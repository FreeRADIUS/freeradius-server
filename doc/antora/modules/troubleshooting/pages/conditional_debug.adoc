...
The following configuration adds a new log destination named `debug`. It should be added to xref:reference:raddb/radiusd.conf.adoc[`radiusd.conf`]. This `debug` destination writes timestamped entries to `default.log` and is only used when specified in unlang.
...
To send log messages to the new log destination, call the xref:reference:xlat/log.adoc#_log_destinationstring_uint32_filename[`%log.destination`] expansion. This expansion sets the destination name, and optionally also the log level and filename.

To log all messages to the new log destination, edit a virtual server (such as the default virtual server, xref:reference:raddb/sites-available/default.adoc[`sites-enabled/default`]) as shown below.
...
Debug logs enabled for every user, all the time, rapidly use up disk space and can affect server performance. The xref:reference:raddb/mods-available/always.adoc[`always`] module provides one method to turn log collection on or off at runtime.

Configure an instance of the `always` module that returns `noop` by default, for example in `mods-enabled/always`:
...
FreeRADIUS restricts which files dynamic expansions can read. Check that the xref:reference:raddb/radiusd.conf.adoc#_security_configuration[security limits] in the `radiusd.conf` allow the server to read the new file:
