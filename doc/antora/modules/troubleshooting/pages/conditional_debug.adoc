...
The following configuration adds a new log destination named `debug`. It should be added to xref:reference:raddb/radiusd.conf.adoc[`raddb/radiusd.conf`]. This `debug` destination writes timestamped entries to `default.log` and is only used when specified in unlang.
...
To log all messages to the new log destination, edit a virtual server (such as the default virtual server, xref:reference:raddb/sites-available/default.adoc[`raddb/sites-enabled/default`]) as shown below.
...
FreeRADIUS restricts which files dynamic expansions can read. Check that the xref:reference:raddb/radiusd.conf.adoc#_security_configuration[security limits] in `raddb/radiusd.conf` allow the server to read the new file:
