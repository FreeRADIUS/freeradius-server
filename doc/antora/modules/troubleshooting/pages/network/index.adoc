...
* does the OS have a firewall which is blocking the application from receiving packets?
  * packet analysers like `wireshark` can often see packets before the firewall drops them!
* is something like SeLinux preventing the application from receiving packets?

All of these should be checked before checking any FreeRADIUS configuration.
...
** xref:network/proxy_state_missing_ma.adoc[Proxy-State without Message-Authenticator]
