...
FreeRADIUS uses OpenSSL to handle SSL operations. Some of its behaviour depends on the version of the OpenSSL libraries it uses and how the global system configuration for OpenSSL is set up.
...
When replacing certificates, ensure that the replacement certificates match any explicit requirements that are configured on the supplicants of all end devices.
...
Distinguished Names trusted by the server which are sent to an end device when a
