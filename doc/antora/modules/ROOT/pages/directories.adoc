...
|===
| Directory               | Description
| `<root>/`               | Sample configuration files for the server.  Can be `/etc/freeradius/` or `/etc/raddb/`
| `mods-available/`       | Module configuration files.
| `mods-enabled/`         | Directory containing symlinks to `raddb/mods-available`.
Controls which modules are enabled.
| `sites-available/`      | Virtual servers.
| `sites-enabled/`        | Directory containing symlinks to `raddb/sites-available`. Control which virtual servers are enabled.
|===
