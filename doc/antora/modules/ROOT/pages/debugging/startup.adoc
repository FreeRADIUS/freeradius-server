...
When the server read the module configuration file above (from `mods-enabled`), that meant just reading the contents of the file.  It is now ready to load the dynamic library which reads that configuration, and which parses the configuration to determine what to do with it.
