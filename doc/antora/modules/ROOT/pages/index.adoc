...
This FreeRADIUS version 4 documentation is available under the
Creative Commons Non-Commercial  xref:LICENSE.adoc[license].

[WARNING]
====
*VERSION 4 IS IN DEVELOPMENT AND HAS NOT BEEN OFFICIALLY RELEASED. USE
AT YOUR OWN RISK.*
...
many options and customisations for the FreeRADIUS server.
Administrators must decide how and what settings to change, which may
...
* xref:getstarted.adoc[Getting Started] guides you through a basic
  install, xref:debugging/radiusd_X.adoc[debugging]. More information
  about where to xref:gethelp.adoc[get help],
...
* xref:reference:index.adoc[Reference documentation] outlines the
  ...
  syntax and xref:reference:raddb/index.adoc[configuration files].
  More configuration information for
...
* xref:howto:index.adoc[Howto Guides] gives step-by-step instructions
  on how to xref:howto:installation/index.adoc[install] FreeRADIUS.
  Many other configuration guides are here for configuring
...
continue to use.  If you have issues, *return* to using FreeRADIUS v3.
...
* Server supports many clients with different shared secrets behind
  one NAT gateway.
...
** The server still doesn't always require (or print) the `::` prefix.
   That will change.
** By implementing `::` for enums, the requirement to use `&` as a
   prefix for attribute names is not needed.
** This change may require an update to all of the configuration.  We
   will try to allow `&`, but that may not be possible.
* Dynamic expansions have changed from `%{md5:foo}` to a more standard
  syntax of `%hash.md5(foo)`.
** The new syntax supports multiple comma-separated arguments such as
   `%function(a, b, c)`.
** Expansions such as `%{User-Name}` work in addition to expressions
   like `%{1+2}` too.
...
read the xref:howto:upgrade/index.adoc[index] guide.  This guide
explains the differences between the two versions and how an existing
configuration is reproduced in the latest release. Do *not* use
version 3 configuration files with version 4. These configuration
...
are available to help you with your software deployments,
