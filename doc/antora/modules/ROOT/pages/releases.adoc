...
Major:: The major version number, e.g. "4" for the foreseeable future.
...
** "point" releases (4.1.1, 4.1.2) _are likely to be incompatible_
   ...
   e.g. A 4.1.1 release _may_ have functionality and behaviour changed
...
etc.  No behaviour changes will be made, and no new core features will
be added.
...
know that its behaviour will never change across "point" releases".
...
behaviour changes, even to fix bugs.  That's where experimental
versions come in.
...
release will note all behaviour changes, including breaking changes in
the release notes.
...
The above release process is necessary to balance a number of
different goals:

* the need to never break existing production deployments,
* the need to add new features, and to fix existing but broken
  features,
...
* new features
* bug fixes which change the behaviour of an existing feature.
...
IMPORTANT: Breaking changes are only permitted IF they are added
behind a feature flag that preserves the previous behaviour by
default.

=== Changes to experimental branches

Minor changes to experimental branches are permitted.  Other work may
need peer review.  Developers must self-classify change sets to
...
- Is it a code area with many untested paths, or poorly structured
  code that's hard to reason about.
- Is the code likely to have widespread impacts on the rest of the
  code base, will it be difficult to change later, will it have
  widespread negative impacts if it's not architected correctly.

.Examples of peer reviewed changes
- Major refactoring over 1000 +/- changes LoC
- Changes to parsing that would break compatibility with previous
  syntax.
- Changes to the behaviour of keywords that are not backwards
  compatible.
...
- Changes to async I/O code other than trivial fixes.  Code paths here
  are notoriously undertested and need careful to review to ensure
...
new syntax.  Changes to behaviour should include a description of
those changes.
