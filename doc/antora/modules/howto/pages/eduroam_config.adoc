...
10. Stop the server.
11. Replace the dummy certs in the ``certs`` directory (``${confdir}/certs``) with ones issued by [edupki](https://www.edupki.org) or a commercial certification authority.
...
server eduroam-inner {
	...
	authorize {
		# The outer username is considered garbage for autz purposes, but
