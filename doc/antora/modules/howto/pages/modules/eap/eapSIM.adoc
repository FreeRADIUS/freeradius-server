...
EAP-SIM configuration in v4 is handled through the `eap-aka-sim` virtual
server (see `raddb/sites-available/eap-aka-sim`). The server calls sections
of this virtual server at each stage of the EAP-SIM exchange.

To supply GSM triplets retrieved from an Authentication Centre (AuC), set
the following `control` attributes inside the `send Challenge-Request` section.
EAP-SIM requires three triplets, so each attribute must be provided three times:
