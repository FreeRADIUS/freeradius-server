...
functionality should instead be marked with a flag, e.g._`string
abinary`.
...
The old-style "tagged" RADIUS format has been removed.  Instead of
using
...
supporting the `Attribute-Name:tag` syntax for tagged attributes. This
change requires modifications to all configuration files and databases
which use tags.  This change means also that `detail` files from v3
are not readable by v4.
