...
section named after Acct-Status-Type. This process is similar to
`authenticate` for `Access-Request` packets. The goal here is to allow
a common pre-processing of accounting packets in the `recv
...
longer necessary.  See the xref:howto:upgrade/unlang.adoc[Unlang
Syntax] pages for more guidance and details.
