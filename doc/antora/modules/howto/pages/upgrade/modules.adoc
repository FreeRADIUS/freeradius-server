...
where the server controls the retransmission behaviour. In v3, the server
retransmitted proxied packets only when it received a retransmission
from the NAS. That behaviour is good, but there are times where
...
like the v3 proxy behaviour of load balancing across home server pools.
...
list, an object is passed to Python functions representing the request.
