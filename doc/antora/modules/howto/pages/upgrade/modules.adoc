...
FreeRADIUS 4 provides new and updated modules that improve its
features and configuration options. This section lists these changes
and highlights any deleted modules. Please check this section to stay
informed about important updates.

FreeRADIUS 4 focuses on easier module management. You can enable
modules by creating soft links from mods-enabled/ to mods-available/.
Another option is to copy the file and edit the configuration files in
mods-enabled/. This method makes customisation simpler and prevents
direct changes to default settings.
...
The `Client-Shortname` attribute has been removed.  You should use
`%client(shortname)` instead.

[#rlm_radius]
=== rlm_radius

The `radius` module has taken over much of the functionality of
`proxy.conf`. See `mods-available/radius` for documentation and
...
The module can send multiple packet types to one home server. e.g.
Access-Request and Accounting-Request.

This module also replaces the old `coa` and `originate-coa`
configuration. See also `subrequest` for creating child requests that
are different from the parent requests.

Unlike v3, the module can do asynchronous proxying. That is, proxying
where the server controls the retransmission behaviour. In v3, the
server retransmitted proxied packets only when it received a
retransmission from the NAS. That behaviour is good, but there are
...
like the v3 proxy behaviour of load balancing across home server
pools.
...
subsections. This behaviour allows for deterministic load-balancing of
...
`control.Cache-Merge` has been renamed to `control.Cache-Merge-New`
and controls whether new entries are merged into the current request.
It defaults to `no`. The primary use case, is if you're using xlat
expansions in the cache module itself to retrieve information for
caching, and need the result of those expansions to be available
immediately.
...
immediately after they are parsed from their ASN1 form.
...
cache. See `sites-available/tls-cache` for a better replacement. The
OpenSSL cache can now be placed on disk, in memory, in memcache, or in
a redis cache. The result is both higher performance, and more
configurable.

The `use_tunneled_reply` and `copy_request_to_tunnel` configuration
items have been removed. Their functionality has been replaced with
the `use_tunneled_reply` and `copy_request_to_tunnel` policies. See
`sites-available/inner-tunnel` and `policy.d/eap` for more
information.
...
server.  See `sites-available/default`.
...
The `expiration` module has been replaced with an `unlang` policy. The
policy is located in `policy.d/time`.  The `Expiration` attribute
should continue to work the same as with v3.

[#rlm_ldap]
=== rlm_ldap

The `ldap` module provides an expansion `%ldap.group(<name>)` instead
of `LDAP-Group` for dynamically testing group membership.  The old
method of
...
The caching of group membership into attributes in the `control` list
...
module, if `cacheable_name` or `cacheable_dn` are enabled.
...
Rather than using the global hashes `%RAD_REQUEST` etc, Perl
subroutines called by FreeRADIUS during packet processing are passed a
hash ref representing the packet being processed.
...
list, an object is passed to Python functions representing the
request. This object has members giving access to all the lists, for
both accessing and setting attributes.

The module now only expects a single return value representing the
module `rcode` since attributes can be set directly within Python
functions.
...
from using flat-text files for `utmp`.  We recommend using sqlite to
store session data, instead of radutmp.

[#rlm_rest]
=== rlm_rest

`REST-HTTP-Code` is now inserted into the `request` list instead of
the `reply.` list.
...
The `reference` option has been removed from `accounting` and
`post-auth` sql module configuration.

Instead, the set of queries run depend on the section the module is
called from.  E.g. calling from `accounting Start` will run the set of
queries defined as
...
behaviour was already supported by the group functionality.  See
rlm_sql and `User-Profile` for new, and superior, behaviour.
...
The following config items must now be defined as attribute
references::
...
Instead, use `attribute-name`. See `mods-available/sqlippool` for more
information.
...
The `winbind` module uses an expansion `%winbind.group(<name>)`
instead of `Winbind-Group == <name>`.
...
This module was poorly documented, and it appears that no one was
using it.
