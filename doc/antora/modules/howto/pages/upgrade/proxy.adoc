...
The proxy feature has undergone massive changes. The `proxy.conf` file
no longer exists, and everything in it has been removed. e.g. `realm`,
...
module. See `mods-available/radius` for examples and documentation.
...
[NOTE]
====
Of course, you will have to use the names of the `radius` modules in
your configuration, and not `home_server_1`, etc.
====
