...
|Retransmits / Loss
|Shows the number of requests that were sent but did not receive responses indicating that the RADIUS server is overwhelmed, most likely due to an authentication dependency (e.g. SQL database, LDAP directory, credential cache) not responding in a timely manner.
...
Similarly, response attributes can be filtered with -R in which case replies are buffered until a response is seen or it times out.
