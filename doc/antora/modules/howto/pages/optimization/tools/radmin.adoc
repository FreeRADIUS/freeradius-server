...
[NOTE]
====
Debug conditions are no longer available in radmin. A more flexible method is available using the xref:reference:xlat/log.adoc[`%log.destination`] expansion xref:troubleshooting:conditional_debug.adoc[directly in unlang].
====
