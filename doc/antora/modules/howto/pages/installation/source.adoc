...
this may cause undesired behaviour and failure to operate correctly.
...
will tell you which configuration files are being used. See
 xref:howto:upgrade/index.adoc[upgrade from v3 to v4] for more information. There _may_ be changes in the
...
For details on what has changed between the version, see  xref:howto:upgrade/index.adoc[upgrade from v3 to v4].
...
you don't, or if you see error messages, please read the xref:ROOT:faq.adoc[FAQ] and xref:troubleshooting:index.adoc[troubleshooting] sections.
