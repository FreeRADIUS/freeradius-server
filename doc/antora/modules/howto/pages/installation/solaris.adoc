...
=== Solaris Packages
Solaris 10 has versions of Openssl and OpenLDAP installed, however they do not fulfill the compile requirements for freeradius functionality.
...
* Many packages are available from [Blastwave](http://www.blastwave.org) which installs everything into the base ``/opt/csw``.
...
#### OpenSSL
Only needed if you want separate OpenSSL libraries
