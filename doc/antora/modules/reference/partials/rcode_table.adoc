...
| `notset`      | The operation isn't finished calculating the `rcode` or the
                `rcode` has not been generated, such as an intial stack state. This internal rcode is used by the interpreter and generally not visible. If this rcode is returned by a module, it usually indicates a bug.
|=====
