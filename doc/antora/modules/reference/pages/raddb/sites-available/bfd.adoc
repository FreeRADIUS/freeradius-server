...
is running. If the FreeRADIUS daemon is down, then we want to know
ASAP.

NOTE: See also `trigger.conf`. There are BFD-specific triggers which
...
Only IP transport is supported for BFD. There is no ethernet type for
BFD.
...
In general the server only needs to know if the BFD session state has
changed. Set this value to "false" if you want every received and
every sent packet to be processed through this virtual server.
...
The peer definition is based on the `client` definition. The standard
client configuration items will work here.
...
source address is given, then a "wildcard" listener will be chosen.
i.e. A listener which has `ipaddr = *`

If the source IP address does not match any listener, then the peer
will be ignored.
...
NOTE: The other side of the BFD connection has to have the same kind
of authentication set.
...
If it starts with "0x", then it is treated as a hex string. This is
recommended for security. The secrets should be no more than 16 octets
long, and random.
...
max_timeouts:: Max number of timeouts before the session is declared
dead.
...
changed*. The contents of the BFD packet are defined by the protocol.
The `send` section is called only for informational purposes.
