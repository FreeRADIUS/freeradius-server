...
Some systems support binding to an interface, in addition to the IP
address. This feature isn't strictly necessary, but for sites with
many IP addresses on one interface, it's useful to say "listen on all
addresses for eth0".

If your system does not support this feature, you will get an error if
you try to use it.
...
See
xref:reference:raddb/mods-available/mac2vlan.adoc[mods-available/mac2vlan]
for the definition of the "mac2vlan" module.
...
NOTE: If you have VLAN's in a database, you can `select` the VLAN name
