...
The module `cui`
(`link:https://freeradius.org/rfc/rfc6572.html#Chargeable-User-Identity[Chargeable-User-Identity]`)
writes
`link:https://freeradius.org/rfc/rfc6572.html#Chargeable-User-Identity[Chargeable-User-Identity]`
log to an SQL database. It uses the `sql` module to do the bulk of the
work, but has custom schemas and queries.
...
[NOTE]
====
For dialect `mysql` and `postgresql` the following information should
be provided:
...
but sometimes it needs to be different. Then instead of editing
`link:https://freeradius.org/rfc/rfc2865.html#User-Name[User-Name]`
(*which we don't recommend*), you can set `SQL-User-Name` to the new
value.
