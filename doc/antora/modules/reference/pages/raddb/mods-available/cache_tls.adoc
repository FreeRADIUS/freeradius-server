...
Also, if an external backend is used, then the TLS session cache can
be shared across multiple RADIUS servers. That makes for simple load
balancing with full session resumption.
...
cache cache_ocsp {
	driver = "rbtree"
	key = %{session-state.TLS-Certificate.Serial}
	update {
