...
will be `yes`. If that IP address does not respond within the timeout,
then the result of the expansion will be `no`.
...
Default is `1` seconds, which should be plenty for nearly anything.
Range is `0.1s` to `10s`.
...
Copy of the `icmp` module, as it may be easier to remember `ping` than
`icmp`.
