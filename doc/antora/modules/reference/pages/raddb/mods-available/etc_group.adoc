...
adds a `Etc-Group-Name` attribute for every group that the user is a
member of.

You will have to define the `Etc-Group-Name` in the `dictionary` file,
as a `string` type. For example:

The field names are just place-holders. They are NOT attribute names!

Add the ETC-Group-Name attribute to the request list. If the user is a
member of multiple groups, then the module will add multiple
