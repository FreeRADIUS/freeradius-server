...
https://tools.ietf.org/html/rfc3580[RFC 3580], and all common RADIUS
practices. Uncommenting the `wimax` module here allows the module to
change the
`link:https://freeradius.org/rfc/rfc2865.html#Calling-Station-Id[Calling-Station-Id]`
attribute to the normal format as specified in
https://tools.ietf.org/html/rfc3580#section-3.21.[RFC 3580 Section
3.21.]
...
MN-HA-PMIP4 = H(MIP-RK, "PMIP4 MN HA" | HA-IPv4 | MN-NAI) MN-HA-CMIP6
...
Some WiMAX equipment requires that the `MS-MPPE-*-Key` attributes are
sent in the Access-Accept, in addition to the `WiMAX-MSK` attribute.
