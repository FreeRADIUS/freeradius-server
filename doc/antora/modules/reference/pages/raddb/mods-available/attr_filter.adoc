...
restricting a client to certain ranges of
`link:https://freeradius.org/rfc/rfc2865.html#Idle-Timeout[Idle-Timeout]`
or `Session-Time-out`.
...
The right-hand side must be the word `ANY`. (This is redundant, as any
A/V Pair not explicitly permitted will be dropped).
...
relaxed:: Only move attribute if it passed all rules, or if the config
says we should copy unmatched attributes.
...
make sure we send back to our RADIUS client only allowed attributes.

Add this after calling `rlm_radius` for proxying.

=== access_reject

Enforce RFC requirements on the contents of
...
packets. Called from the `send Accounting-Response` section.
