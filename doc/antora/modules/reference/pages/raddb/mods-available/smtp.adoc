...
If `recipients` is not set, then messages will be sent to the `TO` address.



cc:: Set the body `CC` header.

If `recipients` is not set, then messages will also be sent to the `CC` addresses.
...
If `recipients` is not set, then messages will also be sent to the `CCC` addresses.
