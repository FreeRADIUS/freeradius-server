...
Each subsection contains values appropriate to the module being called
in the named accounting section.
...
Defaults to LPUSH.  This command will be combined with the key and
insert_arg to produce the insert command.

trim_cmd:: Redis command to run when trimming values.

Defaults to LTRIM.  This command will be combined with the key, 0 and
the value of trim_count to produce the trim command.
