...
Reusable krb5 handles are allocated in blocks. These parameters allow
for tuning how that is done.
...
`radiusd` and that your authorization configuration really uses `krb5`
to do the authentication. You will need to add the following to the
`authenticate` section of your `radiusd.conf` file:
