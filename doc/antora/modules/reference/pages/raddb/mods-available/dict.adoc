...
Note that only the _final_ attribute number is returned. For example,
the OID for `Vendor-Specific.Cisco.AVPair` is `26.9.1`. This function
will return `1`, and not the full OID.
