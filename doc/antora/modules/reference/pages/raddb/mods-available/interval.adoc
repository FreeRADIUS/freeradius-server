...
thread:: Interval limits are tracked per worker thread. Use this for
higher performance when approximate interval limiting is acceptable.
If no key argument is specified in thread mode, then the cost is
reduced further.
