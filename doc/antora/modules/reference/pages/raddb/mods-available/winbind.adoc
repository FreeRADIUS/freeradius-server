...
This module is for PAP authentication (where plaintext passwords are
sent in the
`link:https://freeradius.org/rfc/rfc2865.html#User-Password[User-Password]`
attribute) only. For authenticating `mschap` requests against AD/Samba
see the options available in the `rlm_mschap` module.
...
While not required to be set, it is likely that authentication will
fail if it is not set correctly.
...
This should generally not include a realm, so `Stripped-User-Name` is
likely the best attribute if it exists.

add_domain:: Include the domain in group searches.

When this is enabled, winbind_domain is prepended to the username (as
`domain\username`) before searching. This is generally required.
