...
If an instance is listed in a `session {}` section, this simulates the
user having multilink sessions.
...
The above xlats expand to the current status of the module. To fetch
the current status without affecting it call the xlat with an empty
argument:
