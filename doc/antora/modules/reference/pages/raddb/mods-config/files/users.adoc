...
This behaviour is different from previous versions, where all intermediate changes would remain, even if the module returned `fail`.
...
The assignment operators follow the behaviour of the `user` file, and *do not* follow the new xref:reference:unlang/edit.adoc[editing operators] behaviour.  The reason for this mismatch is that we prefer to not break backwards compatibility for the `users` file.  Both because of how how the operators work, and because using the new-style operators in the `users` format would require changing the `users` file format so much that it would be unrecognizable, and likely not usable.
