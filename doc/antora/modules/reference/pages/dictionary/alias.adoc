...
+
When an `ALIAS` is used at the top of the dictionary, the
`<reference>` is just a name or number.
+
When an `ALIAS` is used to point to another attribute inside of `tlv`,
`struct`, `vsa`, etc., the `<reference>` _must_ begin with a `.`.
This indicates that the ALIAS is to a ``<reference>` with a shared parent.
...
manually enabled on each system by editing the `dictionary`
