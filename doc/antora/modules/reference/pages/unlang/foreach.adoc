...
There is no explicit limit to how many `foreach` statements can be nested. The `foreach` statement uses the stack of the `unlang` interpreter, which can hold up to 64 frames. Each `foreach` statement takes up two frames on the stack. So, the number of nested loops depends on how much of the stack the interpreter is using.
...
The `<key-type>` is the data type for the key or index variable. It can be a numeric type, such as `uint32`, when the `<reference>` is a dynamic expansion or attribute reference. Alternatively, the `<key-type>` can be a `string` type for an attribute reference.
...
An optional data type for the `<value-name>` local variable.  It's usually a 'leaf' data type like `uint32`, or `string`, or `ipv4addr`.  Structural data types like `group` or `tlv` are generally not allowed. However, structural types ARE allowed for attribute references, as long as the structural type exactly matches the attribute's type.
