...
Unlike v3, there is no pre-defined limit as to how many `foreach`
statements can be nested.  However, as with any nested section, the
`foreach` statement uses the stack of the `unlang` interpreter, which
has a limited stack.
...
+
The `<key-type>` is the data type for the key or index variable. It
can be a numeric type, such as `uint32`, when the `<reference>` is a
dynamic expansion or attribute reference. Alternatively, the
`<key-type>` needs to be `string` when the `<reference>` is a
set of attributes that `foreach` is looping over.S
...
increases by one for every round through the loop.  For `string` data
types the key value is the full path to the current attribute being
looped over.
+
...
An optional data type for the `<value-name>` local variable.  It is
usually a 'leaf' data type like `uint32`, or `string`, or `ipv4addr`.
Structural data types like `group` or `tlv` are generally not
allowed. However, structural types ARE allowed for attribute
references, as long as the structural type exactly matches the
...
The name of the local variable which is used to store the current
value when iterating over the attributes.
+
...
When using `foreach` to loop over different data types, the values can
