...
The _<expansion>_ cannot be static data, such as a fixed string. The
switch statement supports xref:xlat/index.adoc[xlat] expansions and
...
Data types which are of the various "integer" or "float" types, or
`ethernet`, or `ifid` are put into a
https://en.wikipedia.org/wiki/Red%E2%80%93black_tree[Red-black
tree]. Entries are matched exactly.
