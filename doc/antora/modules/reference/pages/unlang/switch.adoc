...
The _<expansion>_ cannot be static data, such as a fixed string. The switch statement supports `exec` functions and`xlat` expansions. In addition, any bactick expressions such as `command` can be used here. However, we recommend using the built-in xref:xlat/str/index.adoc[string] manipulation functions for better control and performance.
...
types, the match is done on the network that most closely matches
...
statements are place into a hash table. Entries are matched exactly.

For IP address data types (`ipv4addr`, `ipv6addr`, `ipv4prefix`, `ipv6prefix`, `combo_ip_addr`, and `combo_ip_prefix`), the xref:unlang/case.adoc[case] statements are placed into a https://en.wikipedia.org/wiki/Radix_tree[Patricia / Radix tree]. Multiple networks can be given, including nested networks, so long as there are no duplicates.  Entries are matched on the smallest network which contains the input data.

Data types which are of the various "integer" or "float" types, or `ethernet`, or `ifid` are put into a https://en.wikipedia.org/wiki/Red%E2%80%93black_tree[Red-black tree]. Entries are matched exactly.
...
For compatibility with version 3, an empty `case` statement can also
be used instead of `default`.  This compatibility will be removed in a
future release.

[NOTE]
====
We recommend using the new syntax to avoid processing or performance issues. Replace empty `case` statements with the `default` case syntax in `switch` statements.
====
