...
There must be a dot `.` after the list name and before the attribute name. This syntax helps the server to distinguish between list names and attribute
names.
...
prefixing the _<list>_ name with the `parent` qualifier. If there are multiple
parent-child relationships, the `parent` qualifier can be repeated.

The `outer` key word refers to the outer layer of the EAP conversation, usually the RADIUS request sent by the NAS. In an EAP exchange, `outer` can be referenced as `parent`, since the outer EAP request is a parent of the inner
(tunnelled) request. However, `outer` and `parent` are distinct and the key words `outer` and `parent` are not synonymous. Not every parent is an EAP outer request.
