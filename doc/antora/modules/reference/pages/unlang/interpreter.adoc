...
The `unlang` interpreter allows administrators to write rules which do
the following:
...
language.  It does not permit infinite loops, user-defined functions,
and other general-purpose programming constructs.
...
example, when the server receives an `Access-Request` packet, is runs
the following processing section:
...
describes the larger context of which packets are received, and when
they are received.  This section concentrates on the more narrow topic
`unlang` itself.

== Receiving Requests

When the server receives a packet, the data is decoded and placed as
...
treated as keywords in the language which contain complex behaviour.
For example, the xref:reference:raddb/mods-available/sql.adoc[sql] module has a
complex set of pre-defined behaviour, depending on what kind of packet
it is processing.
...
This abstraction allows the underlying
...
complex set of behaviour.  Please see the
