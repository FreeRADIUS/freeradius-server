...
`recv Access-Request`.  Unlike the xref:unlang/break.adoc[break] statement, which only exits the current xref:unlang/foreach.adoc[foreach] loop, `return` unwinds past any enclosing `foreach` loop and exits the entire processing section or policy.
