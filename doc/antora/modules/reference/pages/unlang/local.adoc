...
For the xref:unlang/redundant.adoc[redundant] and xref:unlang/redundant-load-balance.adoc[redundant-load-balance] keyword, local variables  keep their value across the different subsections.  This behaviour can be used to track states inside of the xref:unlang/redundant.adoc[redundant] section.
...
The variable declaration and assignment must be separated.  Constructions like `uint32 len = %length(%{User-Name})` are not allowed.
