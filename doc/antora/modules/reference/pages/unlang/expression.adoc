...
The following operators are supported (in order).
...
xref:unlang/condition/precedence.adoc[Operator precedence] follows the normal rules. Division by zero means that the entire expression is invalid.
...
the old expressions.  The expression in the previous versions assumed
...
* the logical operators `&&` and `||` return the value which caused them to succeed, e.g. `Foo := (User-Password || "help")` will return the contents of `User-Password` if it exists, otherwise it will return the string `help`.
