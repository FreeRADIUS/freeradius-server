...
not done automatically by the server.  This behaviour is necessary in
...
+
If the _<type>_ field contains a protocol reference such as `@dhcpv4`, `dhcpv4::Discover`, `dhcpv4::Disconnect-Request`, etc., then this creates a new child request of a different protocol.  The rest of the _<type>_ field is then parsed to see what kind of packet is created.
+
