...
given time, it returns `timeout`.

<value>:: A number, or a dynamic expansion, or an attribute reference.
The value is interpreted as a `time_delta`, with default scale in
seconds.
+
The time scale can be changed by appending `s`, `us`, `ms`, `ns`, etc. as
with all uses of `time_delta` values.
+
Negative times, and times less than 1us are treated as zero.  The
`timeout` section does not run, but instead immediately returns the
`timeout` rcode.
...
[NOTE]
====
If the timeout value is taken from a dynamic expansion or attribute
