...
.Example of using unqualified finally
[source,unlang]
----
# Is processed first
recv Access-Request {
...
[NOTE]
====
Requests which are being processed in a `finally` section _will
...
to `finally` sections.  This behaviour is necessary in order for the
...
database is slow, or has an issue?
...
[NOTE]
====
If the `finally` section takes more than 5 seconds, it is forcibly
canceled.  This timeout is hard-coded, and cannot be changed.
====

In order to ensure that the server remains responsive, the `finally`
section should do as little as possible.  In general, it should be
limited to updating some attributes, or logging data to files.

Any potentially long running module calls or dynamic expansions in a
`finally` section should be wrapped in a
xref:unlang/timeout.adoc[timeout] block:
...
stopped, but the child's `finally` section will still run.

Timeouts in `finally` sections of subrequests should therefore
