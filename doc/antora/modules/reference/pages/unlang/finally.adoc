...
nested calls to virtual servers. The `finally` keyword is used internally or within virtual-servers only.
...
[NOTE]
====
If processing is slow or delayed, the 'finally' section can terminate after a pre-defined 5-second timeout and cancel all its children.
====
