...
xref:unlang/elsif.adoc[elsif]). It can also contain
xref:unlang/return_codes.adoc[rcodes] such as `fail` or `ok`.

Most simple keywords can be used inside of a transaction, so long as
they do nothing other than attribute editing and change control flow.
But it is not possible to create a
xref:unlang/subrequest.adoc[subrequest], or
xref:unlang/call.adoc[call] another virtual server.
