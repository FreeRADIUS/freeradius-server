...
In version 4, the `update` statement has been deprecated and no longer
used. The `update` sections can't process nested attributes that are
now supported in version 4.
...
exiting, and returning `fail`.  This behaviour can be overridden with
a xref:unlang/transaction.adoc[transaction], or a
xref:unlang/try.adoc[try] / xref:unlang/catch.adoc[catch] block.
...
| *=       | Repeat a string or octets the given number of times.  The _<rhs>_ must be a positive integer.
