...
In version 4, the `update` statement has been deprecated and no longer used. The
`update` sections can't process nested attributes that are now supported in version 4.  We recommend switching to the new edit syntax, which is more powerful, and less verbose.
...
exiting, and returning `fail`.  This behaviour can be overridden with a xref:unlang/transaction.adoc[transaction], or a xref:unlang/try.adoc[try] / xref:unlang/catch.adoc[catch] block.
...
[NOTE]
====
If the right-hand side of an assignment is a list, then the *only*
...
| !=       | attributes values do not match each other
|=====
...
|          | For `octets`, perform logical "xor".  The value of the _<attribute>_ is "xor"ed with the contents of the _<rhs>_.  Both strings must be of the same length.
...
[NOTE]
====
The `^=` operator behaves differently for `string` and
