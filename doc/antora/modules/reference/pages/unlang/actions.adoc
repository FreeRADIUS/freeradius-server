...
The `actions` block cannot be used for `redundant`, `redundant-load-balance`, `load-balance`, or `parallel` keywords.  Those sections already have default actions to take for the various return codes.  Allowing the `actions` block to over-ride those default actions would cause those sections to operate incorrectly.
...
[NOTE]
====
The `initial_rtx_time` is used for module-level retries and sets the time before the first retransmission attempt. It's not set on section-level retries.
====
