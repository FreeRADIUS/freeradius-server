...
*Goal:* To set up the RADIUS server to communicate with an SQL database by enabling the SQL server and configuring the schema.  This design allows the RADIUS server to retrieve authentication and authorization information directly from the database. It can either replace or work with the `authorize` file.
...
The main configuration for the SQL module is in `mods-available/sql`. It will $INCLUDE the relevant `queries.conf` file for the chosen dialect. We recommend that you review the configuration file for the SQL database you are using.
...
Test the connection manually before proceeding:
