...
dictionary entries, they must go into
xref:reference:raddb/dictionary.adoc[local dictionary] file or into
`dictionary.d/`.

In order to help you organize your own dictionaries, the
`dictionary` file ends with:
...
dictionaries.  See the sample `dictionary` for the `$INCLUDE` directives
...
They live in `dictionary`, never go into a packet, and do not
need a number assigned to them.

Open `dictionary` and add the following lines before the final
...
Create the file `dictionary.d/dictionary.test` with the
following content:
...
The file is picked up automatically because the `dictionary`
...
With `-X`, the server logs one line for the root `dictionary`
...
Edit `mods-config/files/authorize` and add the following entry
for user `bob`:
...
Open `sites-available/default` and look for the `dictionary { }`
...
virtual server without adding anything to the
xref:reference:raddb/dictionary.adoc[local dictionary] file or `dictionary.d/`.
