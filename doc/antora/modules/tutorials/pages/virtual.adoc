...
- `raddb/sites-enabled/virtual`
- `raddb/sites-available/virtual`
...
Create a new file `raddb/sites-available/virtual`:
...
server virtual {
    ...
    listen authentication {
        udp {
            ipaddr = *
            port = 1812 # some random 5 digit number
        }
    }
...
This configuration defines a virtual FreeRADIUS server named `virtual` that listens for authentication requests on UDP port `1812` on all interfaces. The port value can be changed to any available UDP port as required. When an `Access-Request` packet is received, the server sets a cleartext password and processes the request using PAP authentication. The `authenticate pap` section explicitly handles PAP-based authentication by invoking the PAP module.
...
[source,text]
---------
$ cat etc/raddb/sites-enabled/virtual
---------
...
The debug mode shows the server successfully started, is listening on UDP port 1812, and has received an Access-Request for authentication:
