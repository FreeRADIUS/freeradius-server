...
* minimise memory copying
* minimise memory allocation / free
...
An important part of the message implementation is that there are
...
owns the message.  The message target can (temporarily) access the
...
** socket information (likely a pointer to the socket information,
   with src/dst ip/port, file descriptor, etc.)
** ring buffer pointer
...
If the receiver has to sit on the data for a long time (e.g.
`cleanup_delay`), it has to copy the message and any associated packet
data into a separate buffer.  This copy ensures that the message
headers and packet data ring buffer contain information for as short a
...
Since the "create message" API is used only by one caller (e.g.
network thread or worker thread), there is no issue with overlapping /
simultaneous calls to allocate messages.
...
Each message API has one ring buffer associated with it, as above. The
buffer has a fixed size.  The `reader` offset is where messages are
read from.  The `writer` offset is where messages are written to.
...
Data is read into the buffer via an underlying IO implementation.
Then, packets are decoded from the ring buffwr.
...
* used - tell the ring buffer that N bytes of data have been used from
  the ring buffer
* unused - tell the ring buffer than N bytes of data are unused at the
  start.
...
empty, so that the originator can poke the receiver that another
message is ready?
