...
* Log in as root.
* Open a https://www.gnu.org/software/screen/[screen] session: `$
  screen bash`.
...
* When you notice FreeRADIUS has died, reconnect to your screen
  session `$ screen -D -r`.
* At the `(gdb)` prompt type `where` or for _lots_ of info try `thread
  apply all bt full`.
