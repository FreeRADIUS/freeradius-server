...
Coccinelle is an open source tool that can analyse and transform C
code according to specified rules creating SmPL (Semantic Patch
Language) patches. Semantic patches are much more powerful than
patches or regular expressions. so, that's why we use it.
...
* An Introduction to Coccinelle (video)
...
New semantic patches can be proposed and submitted. they should be
organized by name in `scripts/coccinelle/` with good description
inside the script or follow the below tutorial.
