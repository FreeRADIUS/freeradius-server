...
The `modules` header file defines a few macros:
  ...
  USE_YIELD_POINT(inst, name)
  CACHE_DA_YIELD_POINT(name)
  CREATE_DA_YIELD_POINT(inst, name)

Which (in turn):

* defines a named yield point, and updates various other internal macros to be used by `CACHE_DA` and `CREATE_DA`
...
* creates a `fr_dict_attr_t const *da_yield_point_NAME;` inside of the module instance structure.
...
per-request stats, and auto-creates the bins for each yield point.
