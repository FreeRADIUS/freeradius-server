...
** ideally printing out where the request is blocked (module,
   instance, name)
...
** add packets to the "decode" list, which is run at a higher priority
   than other requests.
** the idea here is to quickly clean up the messages between network
   threads and worker threads
** push decoded packets onto the "runnable" priority heap, which is
   ordered by time.
...
DEFINE_YIELD_POINT(name) USE_YEILD_POINT(inst, name)
CACHE_DA_YIELD_POINT(name) CREATE_DA_YIELD_POINT(inst, name)

Which (in turn):

* defines a named yield point, and updates various other internal
  macros to be used by `CACHE_DA` and `CREATE_DA`
...
* creates a `fr_dict_attr_t const *da_yield_point_NAME;` inside of the
...
per-request stats, and auto-creates the bins for each yield point. And
probably also aggregates the stats up the TLV tree.
