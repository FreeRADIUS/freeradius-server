...
Instead, the `unlang` syntax allows for simple if / then / else
checks, and attribute editing.  It does not support recursion,
subroutines, or more complex flow controls.

Where more complicated funactionality is required, we recommend using
the `perl` or `python` modules.  Those modules allow the insertion of
...
use the keywords. Please see the "howto" directory for worked
examples.
...
* link:update.adoc[update] statements are used to edit attributes and
  lists of attributes.
* link:xlat.adoc[sting expansion] Using `%{...}` to perform dynamic
  string expansions.
