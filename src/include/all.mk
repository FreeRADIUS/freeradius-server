...
src/include/%.h: share/dictionary.% share/dictionary.vqp share/dictionary.freeradius.snmp
	${Q}$(ECHO) HEADER $@
	${Q}grep ^ATTRIBUTE $<  | awk '{print "FR_"$$2 " " $$3 "	//!< AUTOGENERATED ATTRIBUTE DEFINITION"}' | tr '[:lower:]/-.' '[:upper:]___' | sed 's/^/#define /' > $@
	${Q}grep ^VALUE $<  | awk '{print "FR_"$$2"_VALUE_"$$3 " " $$4 "	//!< AUTOGENERATED VALUE DEFINITION"}' | tr '[:lower:]/-.' '[:upper:]___' | sed 's/^/#define /' >> $@
