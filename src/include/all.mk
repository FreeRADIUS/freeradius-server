...
src/include/%.h: share/dictionary.% share/dictionary.vqp share/dictionary.freeradius.snmp
	@$(ECHO) HEADER $@
	@grep ^ATTRIBUTE $<  | awk '{print "PW_"$$2 " " $$3 "	//!< AUTOGENERATED ATTRIBUTE DEFINITION"}' | tr '[:lower:]' '[:upper:]' | tr -- - _ | sed 's/^/#define /' > $@
	@grep ^VALUE $<  | awk '{print "PW_"$$2"_VALUE_"$$3 " " $$4 "	//!< AUTOGENERATED VALUE DEFINITION"}' | tr '[:lower:]' '[:upper:]' | tr -- - _ | tr -- . _ | sed 's/^/#define /' >> $@
