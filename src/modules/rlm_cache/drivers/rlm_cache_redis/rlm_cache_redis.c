				       ...
				       REQUEST *request, UNUSED void *handle, uint8_t const *key, size_t key_len)
{
	...
	if ((head->lhs->tmpl_da->vendor == 0) && (head->lhs->tmpl_da->attr == PW_CACHE_CREATED)) {
		...
		c->created = head->rhs->tmpl_data_box.date;
	...
	if ((head->lhs->tmpl_da->vendor == 0) && (head->lhs->tmpl_da->attr == PW_CACHE_EXPIRES)) {
		...
		c->expires = head->rhs->tmpl_data_box.date;
					 ...
					 REQUEST *request, UNUSED void *handle, const rlm_cache_entry_t *c)
{
	...
	created_value.tmpl_data_length = sizeof(created_value.tmpl_data_box.date);
	created_value.tmpl_data_box.date = c->created;
	...
	expires_value.tmpl_data_length = sizeof(expires_value.tmpl_data_box.date);
	expires_value.tmpl_data_box.date = c->expires;
