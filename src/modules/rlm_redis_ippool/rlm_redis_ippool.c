					    ...
					    uint32_t expires)
{
	...
	if (reply->elements > 1) {
		...
		case REDIS_REPLY_INTEGER:
		{
			if (ip_map.lhs->tmpl_da->type != PW_TYPE_IPV4_ADDR) {
				...
				tmp.length = sizeof(ip_map.rhs->tmpl_data_box.integer);

				if (value_box_cast(NULL, &ip_map.rhs->tmpl_data_box, PW_TYPE_IPV4_ADDR,
				...
				ip_map.rhs->tmpl_data_box.integer = ntohl((uint32_t)reply->element[1]->integer);
				ip_map.rhs->tmpl_data_length = sizeof(ip_map.rhs->tmpl_data_box.integer);
		...
		case REDIS_REPLY_STRING:
			ip_map.rhs->tmpl_data_box.strvalue = reply->element[1]->str;
	...
	if (reply->elements > 2) {
		...
		case REDIS_REPLY_STRING:
		{
			...
			range_map.rhs->tmpl_data_box.strvalue = reply->element[2]->str;
	...
	if (inst->expiry_attr && (reply->elements > 3)) {
		...
		expiry_map.rhs->tmpl_data_box.integer = reply->element[3]->integer;
		expiry_map.rhs->tmpl_data_length = sizeof(expiry_map.rhs->tmpl_data_box.integer);
					  ...
					  uint32_t expires)
{
	...
	if (reply->elements > 1) {
		 ...
		 */
		case REDIS_REPLY_STRING:
			range_map.rhs->tmpl_data_box.strvalue = reply->element[1]->str;
	...
	if (inst->expiry_attr) {
		...
		expiry_map.rhs->tmpl_data_box.integer = expires;
		expiry_map.rhs->tmpl_data_length = sizeof(expiry_map.rhs->tmpl_data_box.integer);
		expiry_map.rhs->tmpl_data_type = PW_TYPE_INTEGER;
		if (map_to_request(request, &expiry_map, map_to_vp, NULL) < 0) {
			ret = IPPOOL_RCODE_FAIL;
	...
	case POOL_ACTION_UPDATE:
	{
		...
		case IPPOOL_RCODE_SUCCESS:
			...
			if (inst->copy_on_update) {
				...
				ip_rhs.tmpl_data_box.strvalue = ip_str;
