...
static rlm_rcode_t CC_HINT(nonnull) mod_authenticate(UNUSED void *instance, REQUEST *request)
{
	 ...
	 */
	passwd = fr_pair_find_by_num(request->config, 0, PW_DIGEST_HA1, TAG_ANY);
	if (passwd) {
		...
		passwd = fr_pair_find_by_num(request->config, 0, PW_CLEARTEXT_PASSWORD, TAG_ANY);
	}
	if (!passwd) {
 ...
 *	is single-threaded.
 */
extern module_interface_t rlm_digest;
module_interface_t rlm_digest = {
