...
int fr_dhcp_send_raw_packet(int sockfd, struct sockaddr_ll *p_ll, RADIUS_PACKET *packet)
{
	...
	/* fill in IP layer (L3) */
	struct iphdr *iph = (struct iphdr *)(dhcp_packet + ETH_HDR_SIZE);
	iph->version = 4;
	iph->ihl = 5;
	iph->tos = 0;
	iph->tot_len = htons(IP_HDR_SIZE +  UDP_HDR_SIZE + packet->data_len);  
	iph->id = 0;
	iph->frag_off = 0;
	iph->ttl = 64;
	iph->protocol = 17;
	...
	iph->saddr = packet->src_ipaddr.ipaddr.ip4addr.s_addr;
...
RADIUS_PACKET *fr_dhcp_recv_raw_packet(int sockfd, struct sockaddr_ll *p_ll, struct timeval *p_tv, RADIUS_PACKET *request)
{
	...
	bool received = false;
	while (!received && timerisset(p_tv)) {
		FD_ZERO(&read_fd);
		FD_SET(sockfd, &read_fd);
		retval = select(sockfd + 1, &read_fd, NULL, NULL, p_tv);
		...
		if ( retval > 0 && FD_ISSET(sockfd, &read_fd)) {
			...
			if (ph_ip->protocol != 17) DISCARD_RP("IP protocol (%d) != UDP", ph_ip->protocol);
	...
	if (!received) {
		/* nothing interesting received in allotted time. */
		rad_free(&packet);
		return NULL;
	}
	...
	packet->src_ipaddr.ipaddr.ip4addr.s_addr = ph_ip->saddr;
	packet->dst_ipaddr.af = AF_INET;
	packet->dst_ipaddr.ipaddr.ip4addr.s_addr = ph_ip->daddr;
