...
int eaptls_success(eap_handler_t *handler, int peap_flag)
{
	...
	if (tls_session->label) {
		...
		RWDEBUG("(TLS) Not adding MPPE keys because there is no PRF label");
	}
...
static int eaptls_send_ack(eap_handler_t *handler, int peap_flag)
{
	...
	RDEBUG2("(TLS) ACKing fragment, the peer should send more data.");
...
static fr_tls_status_t eaptls_verify(eap_handler_t *handler)
{
	...
	if (eaptls_packet) {
		 ...
		 */
		RDEBUG3("(TLS) Peer sent flags %c%c%c",
	...
	if ((!eaptls_packet) ||
		...
		if (prev_eap_ds && (prev_eap_ds->request->id == eap_ds->response->id)) {
			...
			REDEBUG("(TLS) Received Unexpected ACK - rejection the connection");
	...
	if (TLS_START(eaptls_packet->flags)) {
		REDEBUG("(TLS) Peer sent EAP-TLS Start message (only the server is allowed to do this)");
		return FR_TLS_INVALID;
	}
	...
	if (TLS_LENGTH_INCLUDED(eaptls_packet->flags)) {
		...
		if (frag_len > total_len) {
			RWDEBUG("(TLS) Fragment length (%zu bytes) is greater than TLS record length (%zu bytes)", frag_len,
				total_len);
		}

		RDEBUG2("(TLS) Peer says that the final record size will be %zu bytes", total_len);
		if (TLS_MORE_FRAGMENTS(eaptls_packet->flags)) {
			...
			if (frag_len + 4) {	/* check for wrap, else clang scan gets excited */
				RDEBUG2("(TLS) Expecting %i fragments",
			...
			if (!prev_eap_ds || (!prev_eap_ds->response) || (!eaptls_prev) ||
			    !TLS_MORE_FRAGMENTS(eaptls_prev->flags)) {
			    	RDEBUG2("(TLS) Got first TLS fragment (%zu bytes).  Peer says more fragments "
			...
			RDEBUG2("(TLS) Got additional fragment with length (%zu bytes).  "
			...
			if (tls_session->tls_record_in_recvd_len > tls_session->tls_record_in_total_len) {
				RWDEBUG("(TLS) Total received fragments (%zu bytes), exceeds "
		...
		if (total_len != frag_len) {
			RWDEBUG("(TLS) Peer says no more fragments, but expected data length (%zu bytes) "
				"does not match EAP-TLS data length (%zu bytes)", total_len, frag_len);
		}

		tls_session->tls_record_in_total_len = total_len;
		tls_session->tls_record_in_recvd_len = frag_len;
		RDEBUG2("(TLS) Got all data (%zu bytes)", frag_len);
	...
	if ((eaptls_prev && TLS_MORE_FRAGMENTS(eaptls_prev->flags)) && !TLS_MORE_FRAGMENTS(eaptls_packet->flags)) {
		RDEBUG2("(TLS) Got final fragment (%zu bytes)", frag_len);
		tls_session->tls_record_in_recvd_len += frag_len;
		if (tls_session->tls_record_in_recvd_len != tls_session->tls_record_in_total_len) {
			RWDEBUG("(TLS) Total received record fragments (%zu bytes), does not equal expected "
	...
	if (TLS_MORE_FRAGMENTS(eaptls_packet->flags)) {
		RDEBUG2("(TLS) Got additional fragment (%zu bytes).  Peer says more fragments will follow",
			frag_len);
		tls_session->tls_record_in_recvd_len += frag_len;
		if (tls_session->tls_record_in_recvd_len > tls_session->tls_record_in_total_len) {
			RWDEBUG("(TLS) Total received fragments (%zu bytes), exceeds "
...
static EAPTLS_PACKET *eaptls_extract(REQUEST *request, EAP_DS *eap_ds, fr_tls_status_t status)
{
	...
	if (TLS_LENGTH_INCLUDED(tlspacket->flags) &&
		...
		REDEBUG("(TLS) Invalid EAP-TLS packet received:  Length bit is set, "
	...
	if (TLS_LENGTH_INCLUDED(tlspacket->flags)) {
		...
		if (data_len > MAX_RECORD_SIZE) {
			REDEBUG("(TLS) Reassembled data will be %u bytes, "
	...
	case FR_TLS_MORE_FRAGMENTS_WITH_LENGTH:
		if (tlspacket->length < 5) { /* flags + TLS message length */
			REDEBUG("(TLS) Invalid EAP-TLS packet received: Expected length, got none");
			talloc_free(tlspacket);
			return NULL;
		}
	...
	default:
		REDEBUG("(TLS) Invalid EAP-TLS packet received");
		talloc_free(tlspacket);
		return NULL;
	}
...
static fr_tls_status_t eaptls_operation(fr_tls_status_t status, eap_handler_t *handler)
{
	...
	if ((tls_session->info.version == TLS1_3_VERSION) &&
		...
		if ((handler->type == PW_EAP_TLS) || SSL_session_reused(tls_session->ssl)) {
			...
			RDEBUG("(TLS) Sending final Commitment Message.");
...
fr_tls_status_t eaptls_process(eap_handler_t *handler)
{
	...
	RDEBUG2("(TLS) Continuing EAP-TLS ...");
	...
	if ((status == FR_TLS_INVALID) || (status == FR_TLS_FAIL)) {
		REDEBUG("(TLS) Verification failed with %s", fr_int2str(fr_tls_status_table, status, "<INVALID>"));
	} else {
		RDEBUG2("(TLS) Verification says %s", fr_int2str(fr_tls_status_table, status, "<INVALID>"));
	}
	 ...
	 */
	case FR_TLS_OK:
		RDEBUG2("(TLS) Done initial handshake");
	...
	if ((tlspacket = eaptls_extract(request, handler->eap_ds, status)) == NULL) {
		REDEBUG("(TLS) Failed extracting TLS packet from EAP-Message");
		status = FR_TLS_FAIL;
		goto done;
	}
	...
	if (tlspacket->dlen !=
		...
		talloc_free(tlspacket);
		REDEBUG("(TLS) Exceeded maximum record size");
		status = FR_TLS_FAIL;
		goto done;
	}
	...
	if (tls_session->is_init_finished) {
		...
		if ((status == FR_TLS_MORE_FRAGMENTS) ||
			 ...
			 */
			eaptls_send_ack(handler, tls_session->peap_flag);
			RDEBUG2("(TLS) Init is done, but tunneled data is fragmented");
			status = FR_TLS_HANDLED;
			goto done;
		}
...
#define MAX_SESSION_SIZE (256)
		...
		if (!vps) {
			RWDEBUG("(TLS) No information in cached session %s", buffer);
		} else {
			vp_cursor_t cursor;
			VALUE_PAIR *vp;
			fr_tls_server_conf_t *conf;

			RDEBUG("(TLS) Adding cached attributes from session %s", buffer);
