...
int mysql_read_naslist(char *nastable)
{
	...
	if (!mysql_checksocket("Auth"))
		return -1;
	sprintf(querystr, "SELECT * FROM %s", nastable);
	if (sqltrace)
		DEBUG(querystr);
	mysql_query(MyAuthSock, querystr);
	if (!(result = mysql_store_result(MyAuthSock)) && mysql_num_fields(MyAuthSock)) {
...
/*
 *      Read the nas table into CLIENT struct
 */
...
int mysql_dict_init(void) {
	...
	sprintf(querystr, "SELECT * FROM %s", mysql_dict_table);
	if (sqltrace)
		DEBUG(querystr);
	mysql_query(MyAuthSock, querystr);
	if (!(result = mysql_store_result(MyAuthSock)) && mysql_num_fields(MyAuthSock)) {
		...
		while ((row = mysql_fetch_row(result))) {
			...
			"dict_init: Invalid name length on line %d of dictionary",
				...
				for (v = dictionary_vendors; v; v = v->next) {
					if (strcmp(row[5], v->vendorname) == 0)
						vendor = v->vendorcode;
				}
				if (row[5][0] && !vendor) {
					log(L_ERR|L_CONS,
...
int mysql_read_realms(char *realmtable) {
   ...
   /* Maybe realm.nas should refer exact to either nas.nasname 
      ...
      unique - CS */
   sprintf(querystr, "SELECT realmname, nasname, ipaddr, authport, options FROM %s, %s WHERE nas = nasname OR nas = shortname or nas = ipaddr GROUP BY realmname", realmtable, mysql_nas_table);
   
   if (sqltrace)
     DEBUG(querystr);
   mysql_query(MyAuthSock, querystr);
   
   if (!(result = mysql_store_result(MyAuthSock)) && 
       mysql_num_fields(MyAuthSock)) 
     {
       log(L_ERR,"MYSQL Error: Cannot get result");
       log(L_ERR,"MYSQL error: %s",mysql_error(MyAuthSock));
       mysql_close(MyAuthSock);
	...
	strcpy(c->realm, row[0]);
	strcpy(c->server, row[1]);
	...
	c->striprealm = strcmp(row[4], "nostrip") != 0;
