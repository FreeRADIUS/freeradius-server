...
static const CONF_PARSER module_config[] = {
    { "num_connections", PW_TYPE_INTEGER, offsetof(REDIS_INST, numconnections), NULL, "20"},
    { "hostname", PW_TYPE_STRING_PTR, offsetof(REDIS_INST, hostname), NULL, "127.0.0.1"},
    { "port", PW_TYPE_INTEGER, offsetof(REDIS_INST, port), NULL, "6379"},
...
static int connect_single_socket(REDISSOCK *dissocket, REDIS_INST *inst) {
     ...
     */
    radlog(L_CONS | L_ERR, "rlm_redis (%s): Failed to connect DB handle #%d", inst->xlat_name, dissocket->id);
        ...
        UNUSED RADIUS_ESCAPE_STRING func)
{
    ...
    if (rlm_redis_query(dissocket, inst, querystr)) {

        // Query failed for x reason, release socket and return
