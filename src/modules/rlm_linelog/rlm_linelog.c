				  ...
				  fr_value_box_list_t *args)
{
	...
	if (inst->file.buffer_write) {
		...
		switch(batch_update(&entry, xctx->mctx, call_env, request, vector, i)) {
		...
		case LINELOG_BUFFER_WRITE_YIELD:
			return unlang_xlat_yield(request, batching_xlat_resume, batching_xlat_handle_signal, ~FR_SIGNAL_CANCEL, entry);
		}
...
static unlang_action_t CC_HINT(nonnull) mod_do_linelog_resume(unlang_result_t *p_result, module_ctx_t const *mctx, request_t *request)
{
	...
	if (inst->file.buffer_write) {
		...
		switch(batch_update(&entry, mctx, call_env, request, vector, vector_len)) {
		case LINELOG_BUFFER_WRITE_FAIL:
			RETURN_UNLANG_FAIL;
		...
		case LINELOG_BUFFER_WRITE_YIELD:
			return unlang_module_yield(request, batching_mod_resume, batching_mod_handle_signal, ~FR_SIGNAL_CANCEL, entry);
		}
	}
...
build_vector:
	...
	case TMPL_TYPE_ATTR:
	{
		...
		if (vector_len == 0) {
			...
			ret = batch_update(&entry, mctx, call_env, request, vector_p, vector_len);
...
static int mod_thread_detach(module_thread_inst_ctx_t const *mctx)
{
	...
	if (inst->log_dst == LINELOG_DST_FILE && inst->file.buffer_write) {
		...
		for (file = fr_hash_table_iter_init(thread->file_table, &iter);
		file;
		file = fr_hash_table_iter_next(thread->file_table, &iter)) {
			batching_cleanup(file);
		}
