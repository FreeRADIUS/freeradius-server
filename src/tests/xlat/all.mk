...
$(BUILD_DIR)/tests/xlat/%: $(DIR)/% $(TESTBINDIR)/unit_test_module | $(BUILD_DIR)/tests/xlat build.raddb
	...
	${Q}if ! $(TESTBIN)/unit_test_module -D share/dictionary -d src/tests/xlat/ -r $@.ok -i $< -xx -O xlat_only > $@.log 2>&1; then \
		if [ ! -f "$@.ok" ]; then \
			echo "FAILED: Problems during execution, Crash? exiting!"; \
			exit 666; \
		fi; \
		cat $@.log; \
		echo "./$(TESTBIN)/unit_test_module -D share/dictionary -d src/tests/xlat/ -r $@.ok -i $< -xx -O xlat_only"; \
		exit 1; \
	fi
	${Q}touch $@
