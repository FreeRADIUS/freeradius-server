...
#  Samples per CPU-second.
#  run-stats.json generated by write_run_stats.sh records the same value.
CPUPROFILE_FREQUENCY=1000

echo "INFO: starting freeradius under gperftools at $(date)"
STATUS=0
CPUPROFILE_FREQUENCY="$CPUPROFILE_FREQUENCY" \
...
#  Record the load-generator totals and what produced the profile.
METRIC_KIND=cpu_time SAMPLING_HZ="$CPUPROFILE_FREQUENCY" EXIT_STATUS="$STATUS" DURATION_S="$SECONDS" \
PROFILER_OPTIONS="CPUPROFILE_FREQUENCY=$CPUPROFILE_FREQUENCY %gperftools.start/%gperftools.stop from server.start/server.stop" \
  bash "$(dirname "$0")/write_run_stats.sh" gperftools
