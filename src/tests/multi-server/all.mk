...
#
#  Scripts that the freeradius container runs. start_profiling.sh is the
#  entry point that a converted test template exec's, and it exec's
#  start_<tool>_profiling.sh when PROFILING=yes.
#  A tool in PROFILING_TOOLS without a script fails the build early with
#  "No rule to make target .../start_<tool>_profiling.sh".
#
TEST_MULTI_SERVER_SCRIPT_DIR   := $(DIR)/scripts
TEST_MULTI_SERVER_SCRIPT_NAMES := start_profiling.sh common_profiling.sh \
...
${4}/scripts/start_profiling.sh: $$(TEST_MULTI_SERVER_SCRIPT_DIR)/profiling/start_profiling.sh
	$${Q}mkdir -p $$(@D)
	$${Q}cp $$< $$@
