...
echo User-Name="bob@realm_$HOSTNAME",User-Password="bob",Message-Authenticator=0x00 | radclient -c "$NUM_REQUESTS" test-container-proxy-1 auth testing123 > "/test/containers/client_$HOSTNAME.log"
if [ "$?" -ne 0 ] ; then
