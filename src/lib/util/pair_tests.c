...
static void test_fr_pair_find_by_da_nested(void)
{
	...
	vp3 = fr_pair_find_by_da_nested(&local_pairs, NULL, fr_dict_attr_test_nested_leaf_string);
	TEST_CASE("Find child node in first TLV");
	TEST_CHECK(vp3 == vp1);

	vp3 = fr_pair_find_by_da_nested(&local_pairs, NULL, fr_dict_attr_test_nested_leaf_int32);
	TEST_CASE("Find child node in second TLV");
	TEST_CHECK(vp3 == vp2);

	vp3 = fr_pair_find_by_da_nested(&local_pairs, vp3, fr_dict_attr_test_nested_leaf_string);
	 ...
	 */
	vp4 = fr_pair_find_by_da_nested(&local_pairs, NULL, fr_dict_attr_test_nested_leaf_string);
	TEST_CASE("Find child node in first TLV");
	TEST_CHECK(vp4 == vp1);

	vp4 = fr_pair_find_by_da_nested(&local_pairs, vp4, fr_dict_attr_test_nested_leaf_string);
