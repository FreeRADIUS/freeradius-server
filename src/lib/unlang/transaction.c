...
static unlang_t *unlang_compile_transaction(unlang_t *parent, unlang_compile_ctx_t *unlang_ctx, CONF_ITEM const *ci)
{
	 ...
	 */
	c->actions.actions[RLM_MODULE_FAIL] = MOD_PRIORITY_MIN;
	c->actions.actions[RLM_MODULE_INVALID] = MOD_PRIORITY_MIN;
	c->actions.actions[RLM_MODULE_DISALLOW] = MOD_PRIORITY_MIN;
