 ...
 */
int pairdata_cmp(PW_TYPE a_type, size_t a_len, value_data_t const *a,
 ...
 */
static int pairdata_cidr_cmp_op(FR_TOKEN op, int bytes,
		    ...
		    PW_TYPE b_type, size_t b_len, value_data_t const *b)
{
	...
	case PW_TYPE_IPV4_ADDR:
		...
		case PW_TYPE_IPV4_PREFIX:	/* IPv4 and IPv4 Prefix */
			return pairdata_cidr_cmp_op(op, 4, 32, (uint8_t const *) &a->ipaddr,
	...
	case PW_TYPE_IPV4_PREFIX:		/* IPv4 and IPv4 Prefix */
		switch (b_type) {
		case PW_TYPE_IPV4_ADDR:
			return pairdata_cidr_cmp_op(op, 4, a->ipv4prefix[1],
	...
	case PW_TYPE_IPV6_ADDR:
		...
		case PW_TYPE_IPV6_PREFIX:	/* IPv6 and IPv6 Preifx */
			return pairdata_cidr_cmp_op(op, 16, 128, (uint8_t const *) &a->ipv6addr,
	...
	case PW_TYPE_IPV6_PREFIX:
		...
		case PW_TYPE_IPV6_ADDR:		/* IPv6 Prefix and IPv6 */
			return pairdata_cidr_cmp_op(op, 16, a->ipv6prefix[1],
	...
	default:
	cmp:
		compare = pairdata_cmp(a_type, a_len, a,
				       b_type, b_len, b);
...
int pairlistcmp(VALUE_PAIR *a, VALUE_PAIR *b)
{
	...
	for (a_p = fr_cursor_init(&a_cursor, &a), b_p = fr_cursor_init(&b_cursor, &b);
		...
		ret = pairdata_cmp(a_p->da->type, a_p->length, &a_p->data,
				   b_p->da->type, b_p->length, &b_p->data);
		if (ret != 0) {
