...
struct fr_coord_s {
	...
	fr_message_set_t		**coord_send_ms;	//!< Message sets for coordinator -> worker messages.
...
static void coord_recv_message(void *ctx, void const *data, size_t data_size, fr_time_t now)
{
	...
	DEBUG3("Coordinator %s got message from worker %d for callback %d",
				   ...
				   bool single_thread, uint32_t max_workers)
{
	...
	*coord = (fr_coord_t) {
	...
	if (fr_control_callback_add(&coord->coord_recv_control, FR_CONTROL_ID_COORD_CALLBACK,
	...
	coord->coord_send_rb = talloc_array(coord, fr_ring_buffer_t *, coord->max_workers);
	coord->coord_send_ms = talloc_array(coord, fr_message_set_t *, coord->max_workers);
...
fr_coord_worker_t *fr_coord_attach(TALLOC_CTX *ctx, fr_event_list_t *el, fr_coord_reg_t *coord_reg)
{
	...
	if (fr_control_callback_add(&cw->worker_recv_control, FR_CONTROL_ID_COORD_CALLBACK,
