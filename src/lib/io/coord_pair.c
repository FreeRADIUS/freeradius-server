...
static int _coord_pair_reg_free(fr_coord_pair_reg_t *to_free)
{
	...
	if (to_free->process_module) talloc_free(to_free->process_module);
...
fr_coord_pair_reg_t *fr_coord_pair_register(TALLOC_CTX *ctx, fr_coord_pair_reg_ctx_t *reg_ctx)
{
	...
	*coord_pair_reg = (fr_coord_pair_reg_t) {
	...
	ret = cf_section_parse(coord_pair_reg, &coord_pair_reg->reuse, cs);
	if (ret < 0) {
	fail:
	...
	coord_pair_reg->server_cs = virtual_server_cs(virtual_server);
	coord_pair_reg->process_module = module_instance_alloc(coord_pair_modules, NULL, DL_MODULE_TYPE_PROCESS,
							       reg_ctx->module_name, reg_ctx->module_name, 0);
	if (!coord_pair_reg->process_module) goto fail;

	if (module_bootstrap(coord_pair_reg->process_module) < 0) goto fail;
	if (module_instantiate(coord_pair_reg->process_module) < 0) goto fail;
					 ...
					 fr_time_t now, void *uctx)
{
	...
	packet_type = virtual_server_packet_type_by_cs(coord_pair->coord_pair_reg->server_cs);
	...
	if (request_init(request, REQUEST_TYPE_INTERNAL,
			 (&(request_init_args_t){
				.namespace = virtual_server_dict_by_cs(coord_pair->coord_pair_reg->server_cs)
			 }))) {
	...
	*packet_ctx = (fr_coord_packet_ctx_t) {
	...
	if (unlang_call_push(NULL, request, coord_pair->coord_pair_reg->server_cs, UNLANG_TOP_FRAME) < 0) {
...
/** Create the coord_pair plugin data
 */
 ...
 */
void fr_coord_pair_data_recv(UNUSED fr_coord_t *coord, uint32_t worker_id, fr_dbuff_t *dbuff, fr_time_t now, void *plugin, void *uctx)
{
	fr_coord_pair_reg_t	*coord_pair_reg = talloc_get_type_abort(uctx, fr_coord_pair_reg_t);
	fr_coord_pair_t		*coord_pair = talloc_get_type_abort(plugin, fr_coord_pair_t);
 ...
 *	- NULL on failure
 */
fr_coord_plugin_t *fr_coord_pair_plugin_create(TALLOC_CTX *ctx, fr_coord_t *coord, fr_event_list_t *el,
					       bool single_thread, void *uctx)
{
	fr_coord_plugin_t	*plugin;
	fr_coord_pair_t		*coord_pair;

	MEM(plugin = talloc(ctx, fr_coord_plugin_t));

	*plugin = (fr_coord_plugin_t) {
