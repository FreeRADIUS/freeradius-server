			...
			char const *src, ssize_t src_len, char quote)
{
	...
	case PW_TYPE_BYTE:
	{
		...
		if (src_enumv && *p && !is_whitespace(p)) {
			if ((dval = dict_val_by_name(src_enumv->attr, src_enumv->vendor, src)) == NULL) {
			 ...
			 PW_TYPE type, DICT_ATTR const *enumv, value_data_t const *data, char quote)
{
	...
	print_int:
	{
		...
		if (enumv && (dv = dict_val_by_attr(enumv->attr, enumv->vendor, i))) {
...
print_int:
		...
		if (enumv && (v = dict_val_by_attr(enumv->attr, enumv->vendor, i)) != NULL) {
			a = v->name;
			len = strlen(a);
		} else {
