				     ...
				     fr_dict_attr_t const *da, rlm_components_t comp)
{
	 ...
	 *	error.
	 */
	dval = dict_value_by_name(da->attr, da->vendor, name2);
	if (!dval) {
		talloc_free(ml);
		cf_log_err_cs(cs,
				  ...
				  rbtree_t *components, rlm_components_t comp)
{
	...
	for (modref = cf_item_find_next(cs, NULL);
		...
		if (comp == MOD_AUTHENTICATE) {
			...
			dval = dict_value_by_name(PW_AUTH_TYPE, 0, modrefname);
			if (!dval) {
				/*
...
static int define_type(CONF_SECTION *cs, fr_dict_attr_t const *da, char const *name)
{
	 ...
	 *	create it again.
	 */
	dval = dict_value_by_name(da->attr, da->vendor, name);
	if (dval) {
		if (dval->value == 0) {
	...
	} while (dict_value_by_attr(da->attr, da->vendor, value));

	cf_log_module(cs, "Creating %s = %s", da->name, name);
	if (dict_value_add(name, da->name, value) < 0) {
		ERROR("%s", fr_strerror());
		return 0;
	}
