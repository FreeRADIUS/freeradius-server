		...
		if (!request->password) {
			...
			if (auth_type) {
				snprintf(clean_password, sizeof(clean_password),
					 "<via Auth-Type = %s>",
					 dict_value_name_by_attr(PW_AUTH_TYPE, 0,
...
static int CC_HINT(nonnull) rad_check_password(REQUEST *request)
{
	...
	while ((auth_type_pair = fr_cursor_next_by_num(&cursor, PW_AUTH_TYPE, 0, TAG_ANY))) {
		...
		RDEBUG2("Found Auth-Type = %s", dict_value_name_by_attr(PW_AUTH_TYPE, 0, auth_type));
...
int rad_postauth(REQUEST *request)
{
	...
	if (vp) {
		...
		RDEBUG2("Using Post-Auth-Type %s",
			dict_value_name_by_attr(PW_POST_AUTH_TYPE, 0, postauth_type));
	}
	...
	if (!autz_retry) {
		...
		if (tmp) {
			...
			RDEBUG2("Using Autz-Type %s",
				dict_value_name_by_attr(PW_AUTZ_TYPE, 0, autz_type));
			autz_retry = 1;
			goto autz_redo;
		}
	}
	...
	if (result >= 0 &&
		...
		if (tmp) {
			...
			RDEBUG2("Using Session-Type %s",
				dict_value_name_by_attr(PW_SESSION_TYPE, 0, session_type));
		}
