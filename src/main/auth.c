		...
		if (!request->password) {
			...
			if (auth_type) {
				snprintf(clean_password, sizeof(clean_password),
					 "<via Auth-Type = %s>",
					 fr_dict_value_name_by_da(NULL, auth_type->da, auth_type->vp_integer));
			} else {
...
static int CC_HINT(nonnull) rad_check_password(REQUEST *request)
{
	...
	while ((auth_type_pair = fr_cursor_next_by_num(&cursor, 0, PW_AUTH_TYPE, TAG_ANY))) {
		...
		RDEBUG2("Found Auth-Type = %s", fr_dict_value_name_by_da(NULL, auth_type_pair->da, auth_type));
...
int rad_postauth(REQUEST *request)
{
	...
	if (vp) {
		...
		RDEBUG2("Using Post-Auth-Type %s",
			fr_dict_value_name_by_da(NULL, vp->da, postauth_type));
	}
	...
	if (!autz_retry) {
		...
		if (tmp) {
			...
			RDEBUG2("Using Autz-Type %s",
				fr_dict_value_name_by_da(NULL, tmp->da, autz_type));
			autz_retry = 1;
			goto autz_redo;
		}
	}
	...
	if (result >= 0 &&
		...
		if (tmp) {
			...
			RDEBUG2("Using Session-Type %s",
				fr_dict_value_name_by_da(NULL, tmp->da, session_type));
		}
