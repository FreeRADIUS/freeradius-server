		...
		if (!request->password) {
			...
			if (auth_type) {
				snprintf(clean_password, sizeof(clean_password),
					 "<via Auth-Type = %s>",
					 dict_value_name_by_attr(0, PW_AUTH_TYPE, auth_type->vp_integer));
			} else {
...
static int CC_HINT(nonnull) rad_check_password(REQUEST *request)
{
	...
	while ((auth_type_pair = fr_cursor_next_by_num(&cursor, PW_AUTH_TYPE, 0, TAG_ANY))) {
		...
		RDEBUG2("Found Auth-Type = %s", dict_value_name_by_attr(0, PW_AUTH_TYPE, auth_type));
...
int rad_postauth(REQUEST *request)
{
	...
	if (vp) {
		...
		RDEBUG2("Using Post-Auth-Type %s",
			dict_value_name_by_attr(0, PW_POST_AUTH_TYPE, postauth_type));
	}
	...
	if (!autz_retry) {
		...
		if (tmp) {
			...
			RDEBUG2("Using Autz-Type %s",
				dict_value_name_by_attr(0, PW_AUTZ_TYPE, autz_type));
			autz_retry = 1;
			goto autz_redo;
		}
	}
	...
	if (result >= 0 &&
		...
		if (tmp) {
			...
			RDEBUG2("Using Session-Type %s",
				dict_value_name_by_attr(0, PW_SESSION_TYPE, session_type));
		}
