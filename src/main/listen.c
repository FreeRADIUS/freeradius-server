...
int listen_bootstrap(CONF_SECTION *server, CONF_SECTION *cs, char const *server_name)
{
	...
	if (!((strcmp(value, "control") == 0) ||
		 ...
		 */
		dv = fr_dict_value_by_name(NULL, fr_dict_attr_by_num(NULL, 0, PW_LISTEN_SOCKET_TYPE), value);
		if (!dv) {
			if (fr_dict_value_add(NULL, "Listen-Socket-Type", value, max_listener++) < 0) {
				cf_log_err_cs(cs,
...
int rad_status_server(REQUEST *request)
{
	...
	case RAD_LISTEN_AUTH:
		dval = fr_dict_value_by_name(NULL, fr_dict_attr_by_num(NULL, 0, PW_AUTZ_TYPE), "Status-Server");
		if (dval) {
	...
	case RAD_LISTEN_ACCT:
		dval = fr_dict_value_by_name(NULL, fr_dict_attr_by_num(NULL, 0, PW_ACCT_TYPE), "Status-Server");
		if (dval) {
		 ...
		 */
	case RAD_LISTEN_COA:
		dval = fr_dict_value_by_name(NULL, fr_dict_attr_by_num(NULL, 0, PW_RECV_COA_TYPE), "Status-Server");
		if (dval) {
