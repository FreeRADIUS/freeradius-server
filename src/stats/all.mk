...
#
#  radict also chain-loads the DHCPv4 and DHCPv6 validation libraries
#  while parsing the RADIUS dictionaries (dictionary.rfc9445 has
#  ref=@DHCPv4 / ref=@DHCPv6), so the rules depend on those protocol
#  libraries too or a parallel build can run radict before they exist.
#

#
#  Define the structures and declare the extern variables
#
src/stats/${1}/${2}_stats.h: share/dictionary/${1}/dictionary.stats $(BUILD_DIR)/lib/local/libfreeradius-${1}.la $(BUILD_DIR)/lib/local/libfreeradius-dhcpv4.la $(BUILD_DIR)/lib/local/libfreeradius-dhcpv6.la $(BUILD_DIR)/bin/local/radict$(E)
...
#
src/stats/${1}/${2}_da_def.c: share/dictionary/${1}/dictionary.stats $(BUILD_DIR)/lib/local/libfreeradius-${1}.la $(BUILD_DIR)/lib/local/libfreeradius-dhcpv4.la $(BUILD_DIR)/lib/local/libfreeradius-dhcpv6.la $(BUILD_DIR)/bin/local/radict$(E)
...
#
src/stats/${1}/${2}_da_autoload.c: share/dictionary/${1}/dictionary.stats $(BUILD_DIR)/lib/local/libfreeradius-${1}.la $(BUILD_DIR)/lib/local/libfreeradius-dhcpv4.la $(BUILD_DIR)/lib/local/libfreeradius-dhcpv6.la $(BUILD_DIR)/bin/local/radict$(E)
...
#
src/stats/${1}/${2}_stats.c: share/dictionary/${1}/dictionary.stats $(BUILD_DIR)/lib/local/libfreeradius-${1}.la $(BUILD_DIR)/lib/local/libfreeradius-dhcpv4.la $(BUILD_DIR)/lib/local/libfreeradius-dhcpv6.la $(BUILD_DIR)/bin/local/radict$(E)
	@echo STATS ${1} ${2} - link
