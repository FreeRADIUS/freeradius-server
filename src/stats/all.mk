...
#
#  radict validates dictionaries by chain-loading protocol libraries,
#  and any dictionary can pull in any other protocol via ref=@<proto>,
#  so rather than tracking which dictionaries reference which
#  protocols (a list which silently rots), the stats rules depend on
#  every target in the lib-protocol and lib-util categories declared before
#  this makefile parses.  src/all.mk orders lib/ and protocols/ ahead
#  of stats/, so that is all of them.
#
DICT_STATS_LIB_DEPS := $(addprefix $(BUILD_DIR)/lib/local/,$(call CATEGORY_TARGETS,lib-protocol lib-util))
...
#
src/stats/${1}/${2}_stats.h: share/dictionary/${1}/dictionary.stats $(DICT_STATS_LIB_DEPS) $(BUILD_DIR)/bin/local/radict$(E)
...
#
src/stats/${1}/${2}_da_def.c: share/dictionary/${1}/dictionary.stats $(DICT_STATS_LIB_DEPS) $(BUILD_DIR)/bin/local/radict$(E)
...
#
src/stats/${1}/${2}_da_autoload.c: share/dictionary/${1}/dictionary.stats $(DICT_STATS_LIB_DEPS) $(BUILD_DIR)/bin/local/radict$(E)
...
#
src/stats/${1}/${2}_stats.c: share/dictionary/${1}/dictionary.stats $(DICT_STATS_LIB_DEPS) $(BUILD_DIR)/bin/local/radict$(E)
	@echo STATS ${1} ${2} - link
